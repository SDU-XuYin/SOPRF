...
void BopTest()
{
	...
	u64 numThreads = 1;
	u64 senderSize = setSize, psiSecParam = 40;// , numThreads(1);
	u64 recverSize = setSize;// , numThreads(1);
	...
	BopPsiSender sendPSIs;
	BopPsiReceiver recvPSIs;
	const u64 Ma_size = (u64)(1.2 * setSize);

	vector<block> Ma(Ma_size);
	vector<block> Mb(Ma_size);
	block tmp;
	for (u64 i = 0; i < Ma_size; i++) {
		for (u64 j = 0; j < Ma_size; j++) {
			 tmp= prng.get_block();
			Ma[i] = Ma[i] ^ tmp;
			Mb[j] = Mb[j] ^ tmp;
		}
	}
	cout << "main 生成本地ai和bi完成， main402" << endl;
		std::thread thrd([&]() {
			PRNG prng(bb);
			//sender thread
			sendPSIs.init(senderSize, recverSize, psiSecParam, *sendChls[0], OTSender0, OneBlock);

			cout << "main senderinit finish， main408" << endl;
			sendPSIs.T_sendInput(sendSet, sendChls,Ma,Mb);
			});
		...
		recvPSIs.init(senderSize, recverSize, psiSecParam, recvChls, OTRecver0, ZeroBlock);
		cout << "main recvcer init finish， main408" << endl;
		recvPSIs.T_sendInput(recvSet, recvChls, Ma,Mb);
